...
> **Status: early.** The wire protocol, the interaction recorder, the crash reporter and the client
> for both Flutter and React Native are implemented and tested. The native capture layers
> (screen, voice, screenshot) are scaffolded and need device work. See [Status](#status).
...
| | state |
|---|---|
| wire protocol (session, challenge, staging, report, crash) | **implemented, tested** |
